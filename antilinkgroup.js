...
import { warningMessages, kickMessage } from './TEKS_PERINGATAN/tek_peringatan_antilinkgroup.js'; // Impor pesan peringatan dan pesan kick
import fs from 'fs';

const DATA_FILE = './DATA/UserWarningsGroup.json';
let userWarnings = {};
const respondedMessages = new Set(); // Set untuk melacak pesan yang sudah direspons
...
export const handleAntiLinkGroup = async (Wilykun, message) => {
	...
	if (linkRegex.test(text) && !respondedMessages.has(messageId)) {
		...
		try {
			...
			if (error.message.includes('rate-overlimit')) {
				...
				await handleAntiLinkGroup(Wilykun, message); // Coba lagi
			} else {
				console.error('Gagal menghapus pesan link grup:', error);
			}
		}
	}
};
...
export const listGroupViolators = async (Wilykun, groupId) => {
	const groupMetadata = await retryWithDelay(() => Wilykun.groupMetadata(groupId));
	const groupName = groupMetadata?.subject || 'Grup';
